...
__global__ void reduce_unroll_last_warp(const T* __restrict__ g_idata,
    ...
    // Final warp reduction (no syncthreads)
    if (tid < 32) {
...
T block_reduce_shfl(T val) {
    static __shared__ T shared[32]; // Up to 32 warps per block
...
// TIMING HELPER
// ============================================================
float run_harris_kernel(const char* name,
                        const T* d_in,
                        T* d_tmp,
                        int N,
                        int blockSize,
    ...
    size_t smem = blockSize * sizeof(T);
    ...
    // Copy partials back and sum on CPU
    T* h_tmp = (T*)malloc(gridSize * sizeof(T));
    CHECK_CUDA(cudaMemcpy(h_tmp, d_tmp, gridSize * sizeof(T),
...
int main() {
    ...
    CHECK_CUDA(cudaMalloc(&d_tmp, gridSizeAtomic * sizeof(T)));
